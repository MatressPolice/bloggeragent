...
@pytest.mark.asyncio
async def test_robust_blog_planner_integration_retry_loop():
    with patch("google.adk.agents.llm_agent.LlmAgent.run_async") as mock_agent_run:
        ...
        rc = RunConfig()
        inv_ctx = InvocationContext(session=session, session_service=session_service, invocation_id="123", run_config=rc)
